  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        <button onClick={() => navigate("/notifications")} className="text-xs text-emerald-600 font-medium hover:text-emerald-700 cursor-pointer">View all</button>
      </div>
      <div className="divide-y divide-slate-50">
        {activityFeed.map((item) => {
          const cfg = typeConfig[item.type] || typeConfig.academic;
          return (
            <div key={item.id} onClick={() => navigate("/notifications")} className="flex items-start gap-3 px-5 py-3.5 hover:bg-slate-50 transition-all cursor-pointer">
